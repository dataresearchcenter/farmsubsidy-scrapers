...
def get_empty_spaces(im):
    ...
    empty_spots.append(last_spot)
    return [
        (empty_spots[i], empty_spots[i + 1]) for i in range(0, len(empty_spots) - 1, 2)
    ]
...
class LetterSplitter:
    ...
    def make_letter(self, end_x, new_x=None):
        part = self.image.crop((self.x, 0, end_x, self.h))
    ...
    def advance(self, until):
        while (
            self.ideal_x < until - self.threshold
            and self.current_letter < self.letter_count
        ):
...
class CaptchaSolver:
    ...
    def predict(self, image_data):
        letters = np.array(
            [
                img_to_array(im, data_format="channels_last")
                for im in letters_from_image(image_data)
            ]
        )

        #         predicted = self.model.predict_classes(letters)
        predicted = np.argmax(self.model.predict(letters), axis=-1)
        return "".join(CLASSES[i] for i in predicted)


def run_on_url(cs, url="https://www.sian.it/pubbAimu/Captcha.jpg"):
    import requests
...
def create_app():
    app = Flask(__name__)
    cs = CaptchaSolver(model_path="./agea_11.h5")
    ...
    def predict():
        ...
        try:
            img_bytes = file.read()
            result = cs.predict_from_bytes(img_bytes)
            print(result)
